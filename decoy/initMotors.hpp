#ifndef CH_NTB_DECOY_INITMOTORS_HPP_
#define CH_NTB_DECOY_INITMOTORS_HPP_
...
void homing_motors_actualPos(int socket){
	int err = 0;
...
void homing_motors_endSwitch(int socket){
	int err = 0;
	
// 	for(int i=0;i<nofAxis;i++){
// 		send_nmt(socket, CANOPEN_NMT_MC_CS_START, nodes[i]);
// 		
// 		if((err = homing_faulhaber_motor_endSwitch(socket, nodes[i])) != 0){
// 			throw eeros::EEROSException("homing failed");
// 		}
// 	}
	
	send_nmt(socket, CANOPEN_NMT_MC_CS_START, node_armLeft);
		
	if((err = homing_faulhaber_motor_endSwitch(socket, node_armLeft)) != 0){
		throw eeros::EEROSException("homing failed");
	}
}
...
#endif // CH_NTB_DECOY_INITMOTORS_HPP_  
